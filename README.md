...
# DONE (CSS Issue, remove display: fixed;): Bug 1: Select dropdown doesn't scroll with rest of the page
...
# DONE (The input's parent label was not properly associated with the input. Fix with htmlFor): Bug 2: Approve checkbox not working
...
# DONE (The issue was that on the InputSelect component, the onChange property was not detecting the case when the newValue was equal to EMPTY_EMPLOYEE, which would mean that it goes back to All Employees. We detect this case and then loadAllTransactions() and return to terminate instead.) Bug 3: Cannot select \_All Employees\* after selecting an employee
...
# DONE (The issue was that in the custom hook, usePaginatedTransactions(), when we set the paginated transactions, we aren't including the previous transactions. So, now in the data key, I pass a list where I spread previousResponse.data and response.data.) Bug 4: Clicking on View More button not showing correct data
...
# DONE (The problem was that the loading state that is applied to the InputSelect component was changed to false after fetching all paginated transactions but we only need to wait for all the employee data. So I moved the setIsLoading(false) to be right after await employeeUtils.fetchAll().) Bug 5: Employees filter not available during loading more data
...
# DONE (For Part 1, the problem was that the logic operators used to display the button was not covering the case where we are viewing by employee. So I added && transactionsByEmployee == null. For part 2, we had a similar issue where the logic operators used to display the button were not covering the case where the nextPage property on paginatedTransactions is null [no more data]. So I added && paginatedTransactions && paginatedTransactions.nextPage !== null.) Bug 6: View more button not working as expected
...
# DONE (This was a bit trickier, because the issue was that useState was inside of each TransactionPane component. The problem then is that React tracks each TransactionPane differently. So, what I did is I hoisted the [approvedState, setApprovedState] into the parent component, Transactions. Here, I added a useState but modified it so it would be a JSON with a string key and boolean value. Then, I passed these two constants down to the child TransactionPane. I slightly modified the setApprovedState parameter by spreading the previous values back into it, then only modifying the specific transaction id with the new boolean value.) Bug 7: Approving a transaction won't persist the new value
